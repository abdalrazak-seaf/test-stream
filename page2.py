# -*- coding: utf-8 -*-
"""
Created on Sun Dec 24 19:46:05 2023
...
creds = ServiceAccountCredentials.from_json_keyfile_name('centering-dock-357814-0200fab82cb6.json', scope)
...
#----
sheet3 = client.open("dashboard").get_worksheet(2)  # Replace with your sheet name

data3 = sheet3.get_all_records()  # Gets all the records from the first sheet

# st.read(data)  # Display the data in the app
# Assuming 'data' is the list of dictionaries you got from the Google Sheet
df_proj = pd.DataFrame(data3)
#----------------
# Add a subheader
st.subheader("Running Projects Analysis")
#-----------------------------------------------------------------------------------------------
...
# def generate_suggested_team(df, designation_counts, rate_threshold=0.5):
#     suggested_team = pd.DataFrame()
#     for designation, count in designation_counts.items():
#         if count > 0:
#             eligible_candidates = df[(df['Designation'] == designation) & (df[f'match_rate_Norm_{suffix}'] > rate_threshold)]
#             # Randomly select candidates
#             selected_candidates = eligible_candidates.sample(min(count, len(eligible_candidates)), random_state=42)
#             suggested_team = suggested_team.append(selected_candidates)
#     return suggested_team



# # Button to generate team and calculate match rates
# if st.sidebar.button("Generate Suggested Team"):
#     st.subheader("Suggested Team and Match Rate Analysis")

#     # Generate suggested team
#     suggested_team = generate_suggested_team(test_df, designation_counts)
#     st.write("Suggested Team:", suggested_team[['Employee Name', 'Designation']])

#     # Calculate match rates for the suggested team
#     # Assuming the project code of the upcoming project is known or set to a dummy value
#     upcoming_project_code = "Upcoming_Project_123"
#     suggested_team['Project Code'] = upcoming_project_code
#     # recalculate_match_rates(suggested_team, selected_service_lines, selected_sub_service_lines)

#     # Display match rates and average normalized match rates
#     for ssl in selected_sub_service_lines:
#         suffix = ssl.replace(' ', '_').replace('&', 'and')
#         ssl_columns = ['Employee Name', 'Designation', f'match_rate_{suffix}', f'match_rate_Norm_{suffix}']
#         st.write(f"Match Rate Analysis for {ssl} (Suggested Team):")
#         st.dataframe(suggested_team[suggested_team['Project Code'] == upcoming_project_code][ssl_columns])

#         # Calculate and display the average normalized match rate
#         avg_norm_rate = calculate_average_match_rate_norm(suggested_team, ssl)
#         st.metric(label=f"Average Norm. Rate for {ssl} (Suggested Team)", value=avg_norm_rate)
...
#             # Generate a suggested team with a different random seed each time
#             random_seed = random.randint(1, 10000)
#             suggested_team = generate_suggested_team(test_df, designation_counts, random_state=random_seed)
#             st.write(suggested_team[['Employee Name', 'Designation']])
...
#                 suffix = ssl.replace(' ', '_').replace('&', 'and')
#                 ssl_columns = ['Employee Name', 'Designation', f'match_rate_{suffix}', f'match_rate_Norm_{suffix}']
#                 st.write(f"Match Rate Analysis for {ssl} (Team {i + 1}):")
#                 st.dataframe(suggested_team[suggested_team['Project Code'] == upcoming_project_code][ssl_columns])
